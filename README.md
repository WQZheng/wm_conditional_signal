...
│
├── runs/                              # Trained models & experiment results (included)
│   ├── p2v2/
│   │   ├── RSSM-sig.pt                # Signal-conditioned RSSM (best model)
...
│   │   ├── idm.pkl                    # Calibrated IDM model
│   │   ├── stats.pkl                  # Normalization statistics
│   │   └── results.json               # P2 open-loop evaluation results
│   └── p3/
│       ├── arterial.{net,nod,edg,rou,add,sumocfg}.xml  # SUMO arterial network
│       └── results.json               # P3 closed-loop 3-seed results
│
...
Trained model checkpoints are included in `runs/p2v2/`:

| Model | File | Description | Size |
|-------|------|-------------|------|
| RSSM-sig | `RSSM-sig.pt` | Signal-conditioned RSSM (proposed) | 1.1 MB |
| RSSM-nosig | `RSSM-nosig.pt` | RSSM without signal (ablation) | 1.1 MB |
| LSTM-sig | `LSTM-sig.pt` | Signal-conditioned LSTM baseline | 357 KB |
| LSTM-nosig | `LSTM-nosig.pt` | LSTM without signal baseline | 345 KB |

Each checkpoint contains model weights (PyTorch `state_dict`). Load with:
```python
import torch
model = RSSM(...)  # see models.py for constructor args
model.load_state_dict(torch.load('runs/p2v2/RSSM-sig.pt'))
model.eval()
```
