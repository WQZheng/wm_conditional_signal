...
│   │       ├── signal_timing_csv/            #   32 CSV files (4 intersections × 4 dirs × 2 periods)
...
# 4a. Full reproduction from scratch (needs NGSIM data, ~30 min on GPU)
bash run_all.sh

# 4b. Skip P1 (use included preprocessed data) — retrain + re-evaluate (~20 min)
bash run_all.sh --skip-p1

# 4c. Only re-run P3 closed-loop simulation (uses included models, ~3 min)
bash run_all.sh --skip-p1 --skip-p2
```

### 10.2 Step-by-Step

#### Step 0: Download NGSIM Data (only for P1)
...
#### Step 2: World Model Training + Evaluation (P2)

```bash
# Best configuration (200 epochs, 6D distance-weighted signal)
python src/swamp/run_p2v2.py

# Per-dimension analysis
python src/swamp/analyze_p2.py

# Basic version (50 epochs, 3D signal — superseded)
python src/swamp/run_p2.py
```

#### Step 3: SUMO Closed-Loop Simulation (P3)
...
# Run closed-loop simulation (3 seeds × 3 controllers)
python src/swamp/run_p3.py
```

### 10.3 Using Pre-Trained Models
...
s0 = torch.randn(1, 5)       # normalized initial state
a_seq = torch.randn(1, 10, 2)  # normalized lead actions
phi_seq = torch.randn(1, 10, 6) # distance-weighted signal phases
preds = model.rollout(s0, a_seq, phi_seq)  # (1, 10, 5) normalized predictions

# Denormalize
preds_phys = preds.cpu().numpy() * s_std + s_mean
```

### 10.4 Available Checkpoints

| Model | File | Size | Params | Description |
|-------|------|------|--------|-------------|
| RSSM-sig (v2) | `runs/p2v2/RSSM-sig.pt` | 1.1 MB | 275,845 | Signal-conditioned RSSM (proposed, 200 epochs) |
| RSSM-nosig (v2) | `runs/p2v2/RSSM-nosig.pt` | 1.1 MB | 275,845 | RSSM without signal (ablation) |
| LSTM-sig (v2) | `runs/p2v2/LSTM-sig.pt` | 357 KB | 90,373 | Signal-conditioned LSTM baseline |
| LSTM-nosig (v2) | `runs/p2v2/LSTM-nosig.pt` | 345 KB | 90,373 | LSTM without signal baseline |
| RSSM-sig (v1) | `runs/p2/RSSM-sig.pt` | 264 KB | 70,405 | Earlier version (50 epochs, 3D signal) |
| IDM | `runs/p2v2/idm.pkl` | 53 B | — | Calibrated IDM parameters |
...
| GPU | NVIDIA RTX 4090 (24 GB), CUDA 12.x |
| Python | 3.10.12 |
| PyTorch | 2.x with CUDA |
| SUMO | 1.12.0 |
| CPU/RAM | 256 cores / 976 GB |
